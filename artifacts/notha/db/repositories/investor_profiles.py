...
class InvestorProfileRepository:
    ...
    async def upsert(
        ...
        return await self._db.fetch_val(
            ...
            INSERT INTO investor_profiles
                (user_id, group_id, risk_tolerance, min_investment_amount,
            ...
            RETURNING id
            """,
            user_id, group_id, risk_tolerance,
    ...
    async def list_active(self, group_id: int | None = None) -> list:
        if group_id:
            return await self._db.fetch_all(
                """
                SELECT ip.*, u.name AS user_name, u.phone
                FROM investor_profiles ip
                JOIN users u ON u.id = ip.user_id
                WHERE ip.is_active = TRUE
                  AND (ip.group_id IS NULL OR ip.group_id = $1)
    ...
    async def find_candidates(
        ...
        Filtros aplicados em SQL:
          - Grupo compatível (NULL = aceita qualquer grupo)
        ...
        return await self._db.fetch_all(
            ...
            """,
            group_id, maturity_days, min_risk_int, exclude,
        )
